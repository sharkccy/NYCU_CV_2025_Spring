...
from utils.dataset_utils import DenoiseTestDataset, DerainDehazeDataset, DerainDesnowDataset, TestSpecificDataset
from utils.val_utils import AverageMeter, compute_psnr_ssim
from utils.image_io import save_image_tensor, torch_to_np
...
def test_all_in_one(net, dataset):
    ...
    with torch.no_grad():
        for ([degraded_name], degrad_patch) in tqdm(testloader):
            ...
            restored_patch = net(degrad_patch)
            save_image_tensor(restored_patch, testopt.output_path + degraded_name[0] + '.png')

    for img_path in os.listdir(testopt.output_path):
        if img_path.endswith('.png'):
            img = Image.open(os.path.join(testopt.output_path, img_path)).convert('RGB')
            img_array = np.array(img).transpose(2, 0, 1)
            predictions[img_path] = img_array

    np.savez(testopt.output_npz_path, **predictions)
    print(f"All-in-one test completed, save {len(predictions)} images to {testopt.output_npz_path}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=int, default=4,
                        help='0 for denoise, 1 for derain, 2 for dehaze, 3 for denoise, derain, and dehaze, 4 for all-in-one')
    parser.add_argument('--test_path', type=str, default='data/test/degraded/', help='path to test directory containing degraded/')
    ...
    parser.add_argument('--output_path', type=str, default='output/', help='path to save result images')
    parser.add_argument('--output_npz_path', type=str, default='output/pred.npz', help='path to save pred.npz')
    parser.add_argument('--ckpt_name', type=str, default="model.ckpt", help='inferencing checkpoint name')
    ...
    if testopt.mode == 0 or testopt.mode == 3:
    ...
    if testopt.mode == 0:
        ...
        all_in_one_set = TestSpecificDataset(testopt.test_path)
        test_all_in_one(net, all_in_one_set)
