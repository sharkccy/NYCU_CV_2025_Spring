...
from torchvision.transforms import ToPILImage, Compose, RandomCrop, ToTensor, ColorJitter, RandomHorizontalFlip, RandomVerticalFlip, RandomRotation
import torch

from utils.image_utils import random_augmentation, crop_img, save_image
...
class PromptTrainDataset(Dataset):
    def __init__(self, args):
        ...
        self.de_dict = {'denoise_15': 0, 'denoise_25': 1, 'denoise_50': 2, 'derain': 3, 'dehaze': 4, 'desnow' : 5}
    ...
    def _init_snow_ids(self):
        ...
        temp_ids+= [self.args.derain_dir + id_.strip() for id_ in open(snow)]
        self.rs_ids = [{"clean_id":x,"de_type":5} for x in temp_ids]
        self.rs_ids = self.rs_ids * 120
    ...
    #     return gt_name
    def _get_gt_name(self, degraded_name):
...
class DerainDesnowDataset(Dataset):
    def __init__(self, args):
        super(DerainDesnowDataset, self).__init__()
        self.args = args
        self.sample_ids = []
        self.toTensor = ToTensor()
        self.de_dict = {'derain': 3, 'desnow': 5}  # Change to desnow: 5
        self.save_counter = 0  # 計數器限制保存圖像數量
        self.max_save = 5  # 保存 5 組圖像對
        self.save_dir = "output/visualization/"  # 保存目錄
        ...
        self.crop_only = Compose([
            ToPILImage(),
            RandomCrop(args.patch_size),
        ])

    def _init_ids(self):
        clean_dir = self.args.clean_dir
        degraded_dir = self.args.derain_dir
        for i in range(1, 1601):
            self.sample_ids.append({
                'clean_id': f'{clean_dir}/rain_clean-{i}.png',
                'degraded_id': f'{degraded_dir}/rain-{i}.png',
                'de_type': 3
            })
            self.sample_ids.append({
                'clean_id': f'{clean_dir}/snow_clean-{i}.png',
                'degraded_id': f'{degraded_dir}/snow-{i}.png',
                'de_type': 5  # Change to 5
            })
    ...
    def __getitem__(self, idx):
        ...
        de_id = sample["de_type"]
        clean_id = sample["clean_id"]
        degraded_id = sample["degraded_id"]

        clean_img = crop_img(np.array(Image.open(clean_id).convert('RGB')), base=16)
        degrad_img = crop_img(np.array(Image.open(degraded_id).convert('RGB')), base=16)

        clean_tensor = self.toTensor(clean_img)
        degrad_tensor = self.toTensor(degrad_img)
        
        seed = random.randint(0, 2**32)  # make sure the same seed is used for both augmentations
        if random.random() < 0.2:
            random.seed(seed)
            torch.manual_seed(seed)
            clean_patch_pil = self.augmentation(clean_tensor)

            random.seed(seed)
            torch.manual_seed(seed)
            degrad_patch_pil = self.augmentation(degrad_tensor)
        else:
            random.seed(seed)
            torch.manual_seed(seed)
            clean_patch_pil = self.crop_only(clean_tensor)

            random.seed(seed)
            torch.manual_seed(seed)
            degrad_patch_pil = self.crop_only(degrad_tensor)
        
        # degrad_patch, clean_patch = random_augmentation(*self._crop_patch(degrad_img, clean_img))
        clean_patch = self.toTensor(clean_patch_pil)
        degrad_patch = self.toTensor(degrad_patch_pil)

        clean_name = clean_id.split("/")[-1].split('.')[0]  
        # 保存圖像進行視覺化（前 max_save 組）
        if self.save_counter < self.max_save:
            os.makedirs(self.save_dir, exist_ok=True)
            clean_patch_pil.save(os.path.join(self.save_dir, f"{clean_name}_clean_{self.save_counter}.png"))
            degrad_patch_pil.save(os.path.join(self.save_dir, f"{clean_name}_degrad_{self.save_counter}.png"))
            self.save_counter += 1
...
class TestSpecificDataset(Dataset):
    ...
    def _init_ids(self):
        ...
        for image_file in os.listdir(self.test_path):
            if any(image_file.lower().endswith(ext) for ext in extensions):
                img_path = os.path.join(self.test_path, image_file)
                name = image_file.split('.')[0]  # 移除擴展名，例如 '0.png' -> '0'
                self.degraded_ids.append({
                    'name': name,
                    'degraded_id': img_path
                })
    
    def __getitem__(self, idx):
        img = self.degraded_ids[idx]
        name = img['name']
        degraded_id = img['degraded_id']
        degrad_img = crop_img(np.array(Image.open(degraded_id).convert('RGB')), base=16)
        degrad_patch = self.toTensor(degrad_img)
        return [name], degrad_patch
    
    def __len__(self):
        return len(self.degraded_ids)
